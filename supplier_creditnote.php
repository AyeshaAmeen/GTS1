...
page(_($help_context = "Enter Credit Note"), false, false, "", $js);
//----------------------------------------------------------------------------------------
...
if (isset($_GET['AddedID']))  
{
    ...
    display_note(get_supplier_trans_view_str_consignment($trans_type, $invoice_no, _("View this Invoice")));
...
if (isset($_GET['New']))
{
	...
	{
		unset ($_SESSION['CREDIT']->grn_items);
		unset ($_SESSION['CREDIT']->gl_codes);
		unset ($_SESSION['CREDIT']);
	}
	$_SESSION['CREDIT'] = new supp_trans_credit(ST_SUPPINVOICE, 0, $_GET['get_grn_id']);
	$grn = $_GET['get_grn_id'];
	$_SESSION['CREDIT']->get_grn_id = $grn;

}

//--------------------------------------------------------------------------------------------------
function clear_fields_credit()
{
    global $Ajax;
	unset($_POST['gl_code']);
...
if (isset($_POST['AddGLCodeToTrans']))
{
	...
	$result = get_gl_account_info_credit($_POST['gl_code']);
	...
	{
		$_SESSION['CREDIT']->add_gl_codes_to_trans_credit($_POST['gl_code'], $gl_act_name,
...
function check_data()
{
	...
	if (!$_SESSION['CREDIT']->is_valid_trans_to_post())
	{
	...
	if (!$Refs->is_valid($_SESSION['CREDIT']->reference))
	{
	...
	if (!is_new_reference($_SESSION['CREDIT']->reference, ST_SUPPINVOICE))
	{
	...
	if (!$Refs->is_valid($_SESSION['CREDIT']->supp_reference))
	{
		display_error(_("You must enter a supplier's invoice reference."));
		set_focus('supp_reference');
		return false;
	}
	if (!is_date( $_SESSION['CREDIT']->tran_date))
	{
	...
	elseif (!is_date_in_fiscalyear($_SESSION['CREDIT']->tran_date))
	{
		display_error(_("The entered date is not in fiscal year."));
		set_focus('trans_date');
		return false;
	}
	if (!is_date( $_SESSION['CREDIT']->due_date))
	{
	...
	if (is_reference_already_there($_SESSION['CREDIT']->supplier_id, $_POST['supp_reference']))
...
{
	copy_to_trans($_SESSION['CREDIT']);

	if (!check_data())
		return;

	$invoice_no = add_supp_invoice_credit($_SESSION['CREDIT']);

    $_SESSION['CREDIT']->clear_items_credit();
...
function commit_item_data_credit($n)
{
	if (check_item_data($n))
	{
        $_SESSION['CREDIT']->add_grn_to_trans_credit($n, $_POST['item_code'.$n],
...
if ($id3 != -1)
{
	$_SESSION['CREDIT']->remove_grn_from_trans_credit($id3);
...
if ($id4 != -1)
{
	$_SESSION['CREDIT']->remove_gl_codes_from_trans_credit($id4);
	clear_fields_credit();
	$Ajax->activate('gl_items');
	$Ajax->activate('inv_tot');
}
...
if ($_SESSION["wa_current_user"]->can_access('SA_GRNDELETE'))
{
	...
	if ($id2 != -1) 
	{
		remove_not_invoice_item_credit($id2);
...
{
	$Ajax->activate('gl_items');
	display_quick_entries($_SESSION['CREDIT'], $_POST['qid'], input_num('totamount'), QE_SUPPINV);
...
invoice_header_creditss($_SESSION['CREDIT']);
if ($_POST['supplier_id']=='') 
		display_error(_("There is no supplier selected."));
else
{
	display_grn_items_credit($_SESSION['CREDIT'], 1);
	display_gl_items_credit($_SESSION[' CREDIT'], 1);
	div_start('inv_tot');
	invoice_totals_credit($_SESSION['CREDIT']);
	div_end();
}
...
br();
submit_center('PostInvoice', _("Enter Credit"), true, '', 'default');
br();
